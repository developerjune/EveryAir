...
<html>
    ...
    <body>
        ...
        <form action="adminListMember.do" method="get" onsubmit="return checkValue()">
        	<div id="search_form">
	                    ...
	                    <tr>
	                        ...
	                        <td>
	                        	<select name="keywordType">
	                        		...
	                        		<option value="phonenumber" id="phonenumber">휴대폰번호</option>
	                        	</select>
	                        </td>
	                        <th>검색방식</th>
	                        <td>
	            ...
	            <div id="button">
	            	<input type="hidden" value="Y" name="denied">
        ...
        <div id="deleteButton">
        	...
        	<c:choose>
        		<c:when test="${param.keyword eq null}">
        			<span class="status">회원수: 총 ${pagingDTO.totalCnt}명</span><br>
        		</c:when>
        		<c:otherwise>
        			<span class="status">검색된 회원수: 총 ${pagingDTO.totalCnt}명</span><br>
        		</c:otherwise>
        	</c:choose>
        ...
        <c:forEach var="list" items="${list}">
            <tr>
            	...
            	<td class='list_n_menu'><a href="adminSelectMember.do?no=${list.no}&denied=${list.denied}">수정</a><input type="button" value="삭제" onclick="removeMemberCheck('single', ${list.no}, ${param.pageNo}, '${param.denied}')"></td>
            </tr>
        </c:forEach>
        </table>
        </div>
        <c:choose>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='adminListMember.do?pageNo=1&table=member&denied=${param.denied}'>처음</a>
	        	</c:otherwise>
        	</c:choose>
        	...
        	<c:forEach var="no" begin="${pagingDTO.startViewPageCnt}" end="${pagingDTO.endViewPageCnt}">
        		<c:choose>
        			...
        			<c:otherwise>
        				<a href='adminListMember.do?pageNo=${no}&table=member&denied=${param.denied}'>${no}</a>
        			</c:otherwise>
        		</c:choose>
        	</c:forEach>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='adminListMember.do?pageNo=${pagingDTO.totalPageCnt}&table=member&denied=${param.denied}'>끝</a>
	        	</c:otherwise>
        	</c:choose>
        </div>
        	</c:when>
 <%--------------------------------------------------------------------------------------------------- --%>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='adminListMember.do?pageNo=1&table=member&searchType=${param.searchType}&keywordType=${param.keywordType}&keyword=${param.keyword}&denied=${param.denied}'>처음</a>
	        	</c:otherwise>
        	</c:choose>
        	...
        	<c:forEach var="no" begin="${pagingDTO.startViewPageCnt}" end="${pagingDTO.endViewPageCnt}">
        		<c:choose>
        			...
        			<c:otherwise>
        				<a href='adminListMember.do?pageNo=${no}&table=member&searchType=${param.searchType}&keywordType=${param.keywordType}&keyword=${param.keyword}&denied=${param.denied}'>${no}</a>
        			</c:otherwise>
        		</c:choose>
        	</c:forEach>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='adminListMember.do?pageNo=${pagingDTO.totalPageCnt}&table=member&searchType=${param.searchType}&keywordType=${param.keywordType}&keyword=${param.keyword}&denied=${param.denied}'>끝</a>
	        	</c:otherwise>
        	</c:choose>
        </div>
        	</c:when>
