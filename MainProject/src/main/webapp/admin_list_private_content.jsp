...
<html>
    ...
    <body>
        ...
        <h3 id="title">비공개 컨텐츠 목록</h3>
        <form action="contentSearchList.adminContent" method="get" onsubmit="return checkValue()">
        	<div id="search_form">
	            ...
	            <div id="button">
	            	<input type="hidden" value="N" name="open">
	            	<input type="hidden" value="1" name="pageNo">
	            	<input type="hidden" value="content" name="table">
	                <input type="submit" value="검색"><input type="button" onclick="resetListPrivatePage()" value="초기화">
	            </div>
        	</div>
        </form>
        <div id="deleteButton">
        	...
        	<c:choose>
        		<c:when test="${param.keyword eq null}">
        			<span class="status">컨텐츠수: 총 ${totalCnt}개</span><br>
        		</c:when>
        		<c:otherwise>
        			<span class="status">검색된 컨텐츠수: 총 ${totalCnt}개</span><br>
        		</c:otherwise>
        	</c:choose>
        ...
        <div>
            ...
            <c:forEach var="list" items="${list}">
            	<tr>
	            	...
	            	<td>${list.hit}</td>
	            	<td class='list_n_menu'><a href="selectContent.adminContent?no=${list.no}">수정</a><input type="button" value="삭제" onclick="removeContentCheck('single', ${list.no}, ${param.pageNo}, '${param.open}')"></td>
            	</tr>
        	</c:forEach>
            </table>
            </div>
        ...
        <c:choose>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='contentList.adminContent?pageNo=1&table=content&open=${param.open}'>처음</a>
	        	</c:otherwise>
        	</c:choose>
        	...
        	<c:forEach var="no" begin="${startViewPageCnt}" end="${endViewPageCnt}">
        		<c:choose>
        			...
        			<c:otherwise>
        				<a href='contentList.adminContent?pageNo=${no}&table=content&open=${param.open}'>${no}</a>
        			</c:otherwise>
        		</c:choose>
        	</c:forEach>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='contentList.adminContent?pageNo=${totalPageCnt}&table=content&open=${param.open}'>끝</a>
	        	</c:otherwise>
        	</c:choose>
        </div>
        	</c:when>
 <%--------------------------------------------------------------------------------------------------- --%>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='contentSearchList.adminContent?pageNo=1&table=content&searchType=${param.searchType}&keywordType=${param.keywordType}&keyword=${param.keyword}&open=${param.open}'>처음</a>
	        	</c:otherwise>
        	</c:choose>
        	...
        	<c:forEach var="no" begin="${startViewPageCnt}" end="${endViewPageCnt}">
        		<c:choose>
        			...
        			<c:otherwise>
        				<a href='contentSearchList.adminContent?pageNo=${no}&table=content&searchType=${param.searchType}&keywordType=${param.keywordType}&keyword=${param.keyword}&open=${param.open}'>${no}</a>
        			</c:otherwise>
        		</c:choose>
        	</c:forEach>
        	...
        	<c:choose>
	        	...
	        	<c:otherwise>
	        		<a href='contentSearchList.adminContent?pageNo=${totalPageCnt}&table=content&searchType=${param.searchType}&keywordType=${param.keywordType}&keyword=${param.keyword}&open=${param.open}'>끝</a>
	        	</c:otherwise>
        	</c:choose>
        </div>
        	</c:when>
